...
trait RandomData {
  ...
  def generateDate(min: DateTime, max: DateTime): DateTime = {
    val random = new Random()
    val diff: Long = max.getMillis - min.getMillis
    new DateTime(min.getMillis + (diff * random.nextDouble()).toLong)
  }

  def generateDouble(min: Double, max: Double): Double = ThreadLocalRandom.current().nextDouble(min, max)

  def generateNumeric(length: Int): String = {
    val random = new Random()
